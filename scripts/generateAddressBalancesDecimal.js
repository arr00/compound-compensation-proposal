...
async function getDaiCompensationBalances() {
  ...
  for (let add in effectedAddresses) {
    let decimalValue = Number(effectedAddresses[add]) / 1e18;
    effectedAddresses[add] = String(decimalValue);
  }

  let content = {};
  content.addresses = effectedAddresses;
  ...
  console.log(
    "Address balances succesfully written to generateAddressBalances/addressBalancesDecimal.json"
  );
  process.exit();
}
